...
This repo provides an **ARKit 61 blendshape** version of the [DualTalk](https://huggingface.co/datasets/ZiqiaoPeng/DualTalk_Dataset) dataset. The original FLAME parameters are converted into per-frame 61-dimensional ARKit blendshape weights, so that the data can be directly used for ARKit-rig based rendering and training.
...
Converted ARKit data is saved under `ARKit_npy/{train,test,ood}/` as `(T, 61)` `.npy` files, and per-sample metadata (text / audio / flame / arkit paths) is written to `metadata/*.jsonl`. See [DOWNLOAD.md](./DOWNLOAD.md) for download instructions.
